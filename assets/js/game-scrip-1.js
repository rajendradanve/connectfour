...
function setUpEventHandler(player1Game, player2Game) {
    ...
    if(player1Game === "computer"){
        computerPlayer(activePlayer);
    }else{
        humanPlayer(activePlayer, player1Game, player2Game);
        }
}
...
function activePlayerCoin(playerNumber){
    ...
    let activeCoin = playerNumber ===1? "red-coin" : "yellow-coin";
...
function computerPlayer(activePlayer) {
    ...
    if(isComputerWin === 0){
        ...
        humanPlayer(changePlayer(activePlayer));

    }else if(isComputerWin === 1){
            let Winner = "Player" + activePlayer;
        gameFinish()
...
function humanPlayer(playerNumber) {
    ...
    let humanCoin = activePlayerCoin(playerNumber);
    for (let i = 0; i < 7; i++) {
